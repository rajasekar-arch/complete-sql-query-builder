...
export default function DMLBuilder() {
  ...
  const generateDML = () => {
    ...
    if (type === 'UPDATE') {
      const setClause = validFields.map(f => `${f.name} = '${f.value}'`).join(', ');
      return `UPDATE ${table} SET ${setClause}${where ? ` WHERE ${where}` : ''};`;
    }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">📋 DML Builder (INSERT / UPDATE / DELETE)</h1>
      ...
      <pre className="bg-gray-100 p-4 rounded border whitespace-pre-wrap">{generateDML()}</pre>
